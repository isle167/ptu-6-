...
// ── 티어 정의 ──────────────────────────────
const TIERS = [
  { name: 'BRONZE',   label: '브론즈',   emoji: '🥉', min: 0,    max: 999,  color: '#cd7f32', bg: 'rgba(205,127,50,0.12)',  winThreshold: 300 },
  { name: 'SILVER',   label: '실버',     emoji: '🥈', min: 1000, max: 1999, color: '#a8a9ad', bg: 'rgba(168,169,173,0.12)', winThreshold: 500 },
  { name: 'GOLD',     label: '골드',     emoji: '🥇', min: 2000, max: 3499, color: '#ffd700', bg: 'rgba(255,215,0,0.12)',   winThreshold: 700 },
  { name: 'PLATINUM', label: '플래티넘', emoji: '💎', min: 3500, max: 4999, color: '#00c9ff', bg: 'rgba(0,201,255,0.12)',   winThreshold: 900  },
  { name: 'DIAMOND',  label: '다이아',   emoji: '💠', min: 5000, max: 6999, color: '#6ec6f5', bg: 'rgba(110,198,245,0.14)', winThreshold: 1100 },
  { name: 'MASTER',   label: '마스터',   emoji: '👑', min: 7000, max: Infinity, color: '#a855f7', bg: 'rgba(168,85,247,0.13)', winThreshold: 1300 },
];
...
// ── 게임 점수 → 승패 판정 ────────────────────
// 최종 점수 = 정확도 + WPM + 남은 시간(랭킹 누적) + 콤보
function calcFinalScore(wpm, accuracy, rankingScore, bestCombo) {
  return Math.round((accuracy || 0) + (wpm || 0) + (rankingScore || 0) / 10 + (bestCombo || 0));
}
