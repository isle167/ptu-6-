...
function gopart(){
  ...
  // 정처기/토익의 모든 파트를 섞어서 출제하는 전체파트 카드
  // 랭킹에서는 이 기록이 "전체파트" 랭킹에 따로 들어가고,
  // 동시에 "정처기" 또는 "토익" 통합 랭킹에도 포함된다.
  const allPartItems = pkeys.flatMap(k => (d.parts[k]?.items || []));
  if(allPartItems.length > 0){
    const allEl=document.createElement('div'); allEl.className='part-card';
    allEl.style.borderColor='var(--indigo)';
    allEl.style.background='var(--indigo-t)';
    allEl.classList.add('featured');
    allEl.innerHTML=`<div class="part-card-cnt">${allPartItems.length}문항</div><div class="part-name"><span class="part-icon">👑</span>전체파트</div><div class="part-desc">${pkeys.length}개 파트 전체 랜덤 출제 · 실전 대비 추천</div>`;
...
function isTotalPartPractice(){
  return pendingLabel === '전체파트' && gameMode === 'normal';
}
...
function renderPracticeCountButtons(){
  ...
  if(!wrap) return;
  wrap.innerHTML = '';
  for(let n = 40; n <= 100; n += 10){
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'btn-indigo practice-count-btn';
    btn.textContent = n + '문제';
    btn.onclick = () => choosePracticeCount(n);
    wrap.appendChild(btn);
  }
  highlightPracticeCount();
}

function highlightPracticeCount(){
  document.querySelectorAll('.practice-count-btn').forEach(btn => {
    const active = btn.textContent === practiceQuestionCount + '문제';
    btn.style.background = active ? 'var(--indigo)' : 'transparent';
    btn.style.color = active ? 'white' : 'var(--indigo)';
  });
}

function choosePracticeCount(count){
  practiceQuestionCount = count;
  highlightPracticeCount();
  updateStartBtn();
}

function updatePracticeCountBox(){
  const box = document.getElementById('practice-count-box');
  if(!box) return;
  box.style.display = isTotalPartPractice() ? 'block' : 'none';
  if(isTotalPartPractice()) renderPracticeCountButtons();
}
...
function openModeSelect(items,label){
  ...
  const countBox = document.getElementById('practice-count-box'); if(countBox) countBox.style.display='none';
  ...
  updatePracticeCountBox(); updateStartBtn(); showScreen('s-mode', false);
}
...
function startGame(items,label){
  ...
  if(gameMode === 'ranking') words = shuffledItems.slice(0, 30);
  else if(gameMode === 'normal' && label === '전체파트') words = shuffledItems.slice(0, Math.min(practiceQuestionCount, shuffledItems.length));
  else words = shuffledItems;
...
function getWordDisplayLength(w){
  // 글자 수: 공백 제외 실제 글자 수 반환 (힌트 언더바 수와 일치)
  return [...w].length;
}

function buildLetterHintDisplay(w){
  // 글자 수만큼 언더바 표시 (공백은 공백으로, 나머지는 _)
  return [...w].map(ch => ch === ' ' ? ' ' : '_').join(' ');
}
...
function loadWord(){
  ...
  const wordLen = getWordDisplayLength(item.w);
  if (gameMode === 'timeattack' || gameMode === 'ranking') { 
    hLenBtn.style.display = 'none'; answerHintBtn.style.display = 'none'; 
  } else { 
    hLenBtn.style.display = 'inline-block'; 
    hLenBtn.innerHTML = `💡 글자수 힌트 <span class="hint-count-badge">${wordLen}글자</span>`; 
    hLenBtn.disabled = false; 
    answerHintBtn.style.display = 'inline-block'; 
    answerHintBtn.classList.remove('active'); 
    answerHintBtn.textContent = '💡 정답 힌트'; 
  }
  ...
  // 힌트 언더바 표시 (글자 수와 동일하게)
  const hFirst = document.getElementById('h-first');
  if(hFirst){ hFirst.style.display='none'; hFirst.textContent=''; }
...
function showAnswerFeedback(type, item, inputValue) {
  const panel = document.getElementById('answer-feedback');
  if (!panel || !item) return;
  const safe = value => (typeof escapeHtml === 'function') ? escapeHtml(value) : String(value ?? '');
  panel.className = 'answer-feedback show ' + type;
  panel.innerHTML = type === 'correct'
    ? `<b>정답</b><span>${safe(item.w)}</span><small>${safe(item.m)}</small>`
    : `<b>오답</b><span>정답: ${safe(item.w)}</span><small>입력: ${safe(inputValue || '-')}</small>`;
  clearTimeout(panel._timer);
  panel._timer = setTimeout(() => panel.classList.remove('show'), 1700);
}
...
function submitAnswer(val){
  ...
  if(wordsMatch(val.trim(), w)){
    showAnswerFeedback('correct', words[idx], val.trim());
    ...
    showAnswerFeedback('wrong', words[idx], val.trim());
    showMascotReaction('wrong');
...
function showLenHint() {
  if(gameMode==='timeattack' || gameMode==='ranking')return;
  playSound(sfx.button);
  const item = words[idx];
  const wordLen = getWordDisplayLength(item.w);
  const underbar = buildLetterHintDisplay(item.w);
  const hLenBtn = document.getElementById('h-len');
  hLenBtn.innerHTML = `✅ ${wordLen}글자: <span style="font-family:monospace;letter-spacing:2px;">${underbar}</span>`;
  hLenBtn.disabled = true;
  hintUsed = true;
  combo = 0;
  document.getElementById('g-combo').textContent = '0';
  document.getElementById('h-combo').style.display = 'none';
...
async function endGame(){
  ...
  if (gameMode === 'ranking' && totalWords > 0 && typeof saveRankingRecord === 'function') {
    if (typeof processTierResult === 'function') {
      tierResult = processTierResult({ wpm, accuracy: acc, rankingScore, bestCombo });
    }
  ...
  if (typeof addDailyProgress === 'function' && totalWords > 0) addDailyProgress(totalWords);

  if (totalChars === 0) playSound(sfx.over_bad); else if (acc === 100) playSound(sfx.over_perfect); else if (acc <= 30) playSound(sfx.over_bad);

  // 결과 아이콘 - 랭킹전 승패 반영
  const isRankWin = tierResult?.isWin;
  document.getElementById('res-title').textContent = isTimeOver ? 'TIME OVER' : (gameMode === 'ranking' ? (isRankWin ? '승리!' : '패배') : 'Result');
  document.getElementById('res-icon').textContent = isTimeOver ? '⏰' : (gameMode === 'ranking' ? (isRankWin ? '🏆' : '💀') : (acc>=90?'🎉':acc>=70?'👍':'💪'));
  ...
  // 티어 결과 패널 삽입 (랭킹전일 때만)
  if (tierResult && typeof renderTierResultPanel === 'function') {
    renderTierResultPanel(tierResult);
  }
...
// ============================================
// 재미 요소: 스트릭 격려 & 마스코트 반응
// ============================================

const STREAK_MESSAGES = [
  { at: 3,  emoji: '🐱', msg: '냐옹~ 순조롭게 시작했어요!' },
  { at: 5,  emoji: '🔥', msg: '5연속! 탄력 붙었는데요?' },
  { at: 10, emoji: '⚡', msg: '10연속!! 완전 집중 모드!' },
  { at: 15, emoji: '🦁', msg: '15연속!!! 당신은 타이핑 사자!!' },
  { at: 20, emoji: '🚀', msg: '20연속!!!! 로켓 발사급 실력!' },
  { at: 30, emoji: '👑', msg: '30연속!!!!! 진짜 전설이에요!' },
];

function showStreakEncouragement(){
  if(gameMode === 'timeattack' || gameMode === 'ranking') return;
  const match = STREAK_MESSAGES.find(m => combo === m.at);
  if(!match) return;

  const el = document.createElement('div');
  el.className = 'streak-toast';
  el.innerHTML = `<span class="streak-emoji">${match.emoji}</span><span class="streak-msg">${match.msg}</span>`;
  document.getElementById('root').appendChild(el);

  requestAnimationFrame(() => el.classList.add('streak-toast-in'));
  setTimeout(() => { el.classList.remove('streak-toast-in'); el.classList.add('streak-toast-out'); setTimeout(() => el.remove(), 400); }, 2200);
}

function showMascotReaction(type){
  // 정답/오답에 따른 마스코트 이모지 반응
  const reactions = {
    correct: ['😸','😻','🐱','✨','🎉'],
    wrong:   ['😿','😾','🙀','💦','😅'],
    hint:    ['🤔','💡','🐾','👀'],
  };
  const list = reactions[type] || reactions.correct;
