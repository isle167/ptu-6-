...
function updateTimerUI(){ document.getElementById('g-timer').textContent = gameMode==='timeattack' ? formatTime(remainingTime) : '∞'; }
function startTimer(){
  clearInterval(timerInterval); updateTimerUI();
  timerInterval=setInterval(()=>{
    remainingTime--;
    if(remainingTime<=0){ remainingTime=0; updateTimerUI(); clearInterval(timerInterval); endGame(); return; }
    updateTimerUI();
  },1000);
}
...
function openModeSelect(items,label){
  ...
  ['normal','timeattack'].forEach(m => { const el = document.getElementById('mode-card-' + m); if(el) el.classList.remove('selected'); });
...
function chooseMode(mode){
  gameMode=mode;
  ['normal','timeattack'].forEach(m => { const el = document.getElementById('mode-card-' + m); if(el) el.classList.toggle('selected', m === mode); });
  const box=document.getElementById('time-select-box');
  if(mode==='timeattack'){
    box.style.display='block'; timeLimit=0;
    document.querySelectorAll('#time-select-box .btn-indigo').forEach(btn => { btn.classList.remove('active-time'); btn.style.background = ''; btn.style.color = ''; });
  } else { box.style.display='none'; timeLimit=0; remainingTime=0; }
  updateStartBtn();
}
...
function updateStartBtn() {
  ...
  btn.disabled = !(mixMode && (gameMode === 'normal' || (gameMode === 'timeattack' && timeLimit > 0)));
}
...
function startGame(items,label){
  ...
  if(gameMode==='timeattack') remainingTime=timeLimit||60; else { timeLimit=0; remainingTime=0; }
  updateTimerUI();
...
function loadWord(){
  ...
  if (gameMode === 'timeattack') { hLenBtn.style.display = 'none'; answerHintBtn.style.display = 'none'; } 
